...
OpenClaw comme substrat d’exécution gouverné
Le système cible ne doit pas être conçu comme un remplacement total d’OpenClaw, mais comme une couche supérieure de gouvernance missionnelle construite au dessus de son moteur d’exécution. OpenClaw apporte déjà des primitives d’exécution configurables, notamment des permission modes, des exec approvals, des allowlists et des mécanismes de pairing. Selon la configuration active, il est donc partiellement autonome par défaut sur certaines classes d’actions, et peut devenir nettement plus autonome lorsque l’opérateur choisit des réglages plus permissifs.
Ces primitives doivent être traitées comme un Execution Control Substrate. Elles bornent ce que le moteur peut lire, écrire, exécuter ou pairer, mais elles ne suffisent pas, à elles seules, à porter la gouvernance missionnelle visée par ce document. Elles ne remplacent ni la Mission Charter, ni le Case File, ni les Decision Records, ni les Evidence Bundles, ni les mécanismes de recovery, ni Mission Control.
Il faut distinguer explicitement deux niveaux de gouvernance. Le premier niveau est la gouvernance d’exécution immédiate, déjà partiellement prise en charge par OpenClaw à travers ses permissions, approvals, allowlists, pairings et mécanismes de sandboxing. Le second niveau est la gouvernance missionnelle persistante, portée par le produit cible, qui organise la trajectoire de mission, les objets gouvernés, la mémoire vivante, la reprise, les records, les preuves, la supervision utile et la continuité dans le temps. OpenClaw ne remplace pas cette seconde couche. Il en constitue le moteur d’exécution sous-jacent.
En conséquence, l’architecture cible doit être pensée en couches. OpenClaw reste le moteur et la couche immédiate de permissions d’exécution. Le Mission Coordinator porte la coordination persistante de mission. PostgreSQL porte la source de vérité canonique. Mission Control porte la supervisabilité utile. La gouvernance produit, enfin, borne et module l’autonomie effective au niveau de la trajectoire missionnelle.
Les états natifs d’approval, de permission, de pairing et de refus d’exécution d’OpenClaw doivent remonter comme signaux et comme preuves dans le système supérieur. Ils doivent pouvoir alimenter les records décisionnels pertinents, enrichir le Case File, et être projetés dans Mission Control lorsque leur impact devient significatif pour la mission ou pour l’intervention humaine.
L’autonomie du produit doit donc être comprise comme une autonomie composée. La couche missionnelle gouverne la trajectoire, les preuves, la reprise et la convergence. OpenClaw gouverne l’enveloppe immédiate d’exécution selon ses règles de permission et d’approbation. L’enjeu n’est pas de dupliquer aveuglément ses garde-fous, mais de les intégrer, de les rendre visibles, et de les relier explicitement à la gouvernance de mission.
...
Cette hiérarchie ne doit pas être lue comme une simple re-labellisation des agents ou sous-agents déjà présents dans OpenClaw. Elle constitue une structure institutionnelle propre au produit, qui peut s’appuyer sur les primitives du moteur existant, mais qui conserve une autonomie conceptuelle et normative. Autrement dit, le produit réutilise le substrat OpenClaw sans réduire sa propre gouvernance à l’architecture interne du moteur.
...
Il faut en conséquence refuser toute confusion entre run et mission. Un run OpenClaw ne constitue pas une mission enrichie, mais un épisode d’exécution situé à l’intérieur d’une mission plus large. Une mission peut contenir, traverser ou réactiver plusieurs runs successifs ou parallèles sans perdre son identité propre. Le run relève principalement du substrat d’exécution. La mission relève de la couche de gouvernance persistante.
...
La Mission State Projection ne doit pas être comprise comme une simple interprétation opportuniste côté interface. Elle doit rester une projection backend ou persistée issue de l’état canonique, optimisée pour Mission Control mais gouvernée au même titre que les autres objets structurants. L’interface peut la reformater, mais elle ne doit pas l’inventer.
...
Le onzième chantier concerne le mapping explicite entre les primitives natives d’OpenClaw, notamment les permission modes, exec approvals, pairing approvals, allowlists et états d’autorisation, et la couche supérieure de gouvernance missionnelle décrite par la V2. Il reste à formaliser quels signaux doivent être remontés dans le Case File, quels états doivent être projetés dans Mission Control, quelles décisions produit s’appuient directement sur la policy OpenClaw, et où la gouvernance de mission doit compléter ou surdéterminer les garde-fous déjà présents dans le moteur.
